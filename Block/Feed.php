...
class Feed extends Template
{
    ...
    private const XML_PATH_IG_BUSINESS_ID = 'elie_instagram/general/instagram_business_id';
    ...
    private function getInstagramBusinessId(): ?string
    {
        return $this->_scopeConfig->getValue(
            self::XML_PATH_IG_BUSINESS_ID,
            ScopeInterface::SCOPE_STORE
        ) ?: '17841401271373980'; // RedLine Instagram Business ID
    }

    /**
     * Get Instagram posts via Graph API
     */
    public function getPosts(): array
    {
        ...
        try {
            $igUserId = $this->getInstagramBusinessId();
            
            $url = sprintf(
                'https://graph.facebook.com/v18.0/%s/media?fields=id,media_type,media_url,permalink,thumbnail_url&access_token=%s&limit=%d',
                $igUserId,
                $accessToken,
                $this->getLimit()
            );
            ...
            if (isset($data['data']) && is_array($data['data'])) {
                return array_map(function($item) {
                    // Pour les vidéos, utiliser thumbnail_url comme media_url
                    if ($item['media_type'] === 'VIDEO' && isset($item['thumbnail_url'])) {
